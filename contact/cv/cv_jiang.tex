...
%\item[] @Anuyah, O., @Douailly-Backman, J.-P., Metoyer, R., \textbf{Jiang, M.} ``Modeling User Behaviors to Predict HatefulnessLevel of Posts on Social Media'', under review at \textit{???} (???), 2020.

\item[CR11] Liu, Z., \textbf{Jiang, M.}, Luo, T. ``Leverage Electron Properties to Predict Phonon Properties via Transfer Learning for Semiconductors'', under review at \textit{ASME's International Mechanical Engineering Congress and Exposition} (IMECE), 2020. (abstract)

\item[CR10] @Zhao, T., Liu, Y., Woodford, O., Neves, L., \textbf{Jiang, M.}, Shah, N. ``Improving Graph Neural Network Generalization with Data Augmentation'', under review at \textit{Conference on Neural Information Processing Systems} (NeurIPS), 2020.
...
\item[CR8] @Yu, W., Wu, L., Deng, Y., Mahindru, R., @Zeng, Q., Guven, S., \textbf{Jiang, M.} ``A Technical Question Answering System with Transfer Learning'', under review at \textit{Conference on Empirical Methods in Natural Language Processing} (EMNLP), 2020. (demo)
	
\item[CR7] @Yu, W., Wu, L., Deng, Y., Mahindru, R., @Zeng, Q., Guven, S., \textbf{Jiang, M.} ``Technical Question Answering across Tasks and Domains'', under review at \textit{Conference on Empirical Methods in Natural Language Processing} (EMNLP), 2020.
